    ...
    describe("SupportStatement", function () {
        ...
        describe("#supportedBy", function () {
            it("returns an array of releases represented by the statement", function () {
                const st = new SupportStatement({ version_added: "1" }, browser("chrome"));
            ...
            it("handles ≤ gracefully", function () {
                const st = new SupportStatement({ version_added: "≤11" }, browser("chrome"));
                const rels = st.supportedBy();
        ...
        describe("supportedIn()", function () {
            ...
            it("throws when release does not correspond to the statement's browser", function () {
                const statement = new SupportStatement({ version_added: "1" }, browser("chrome"));
                assert.throws(() => statement.supportedInDetails(browser("firefox").current()), Error);
            });
            it("returns supported when release is on after version_added", function () {
                ...
                assert.equal(unranged.supportedInDetails(cr.version("100")).supported, true);
                assert.equal(unranged.supportedInDetails(cr.version("101")).supported, true);
                assert.equal(unranged.supportedInDetails(cr.current()).supported, true);
                assert.equal(unranged.supportedInDetails(cr.releases.at(-1)).supported, true);
                assert.equal(ranged.supportedInDetails(cr.version("99")).supported, null);
                assert.equal(ranged.supportedInDetails(cr.version("100")).supported, true);
                assert.equal(ranged.supportedInDetails(cr.version("101")).supported, true);
                assert.equal(ranged.supportedInDetails(cr.current()).supported, true);
                assert.equal(ranged.supportedInDetails(cr.releases.at(-1)).supported, true);
            });
            it("returns supported when release is on after version_added and before version_removed", function () {
                const cr = browser("chrome");
                const unranged = new SupportStatement({ version_added: "100", version_removed: "125" }, cr);
                const ranged = new SupportStatement({ version_added: "≤100", version_removed: "125" }, cr);
                assert.equal(unranged.supportedInDetails(cr.version("99")).supported, false);
                assert.equal(unranged.supportedInDetails(cr.version("100")).supported, true);
                assert.equal(unranged.supportedInDetails(cr.version("101")).supported, true);
                assert.equal(unranged.supportedInDetails(cr.version("124")).supported, true);
                assert.equal(unranged.supportedInDetails(cr.version("125")).supported, false);
                assert.equal(ranged.supportedInDetails(cr.version("99")).supported, null);
                assert.equal(ranged.supportedInDetails(cr.version("100")).supported, true);
                assert.equal(ranged.supportedInDetails(cr.version("101")).supported, true);
                assert.equal(ranged.supportedInDetails(cr.version("124")).supported, true);
                assert.equal(unranged.supportedInDetails(cr.version("125")).supported, false);
            });
            it("returns unknown support when release is before ranged version_added", function () {
                ...
                const rangedClosed = new SupportStatement({ version_added: "≤100", version_removed: "125" }, cr);
                assert.equal(rangedOpen.supportedInDetails(cr.version("99")).supported, null);
            ...
            it("returns unknown support when release is after version_added and before ranged version_removed", function () {
                const cr = browser("chrome");
                const rangedEnd = new SupportStatement({ version_added: "100", version_removed: "≤125" }, cr);
                assert.equal(rangedEnd.supportedInDetails(cr.version("100")).supported, true);
                assert.equal(rangedEnd.supportedInDetails(cr.version("101")).supported, null);
                assert.equal(rangedEnd.supportedInDetails(cr.version("124")).supported, null);
                assert.equal(rangedEnd.supportedInDetails(cr.version("125")).supported, false);
            });
            ...
            it("returns unsupported when release is on or after version_removed", function () {
                const cr = browser("chrome");
                const unranged = new SupportStatement({ version_added: "1", version_removed: "10" }, cr);
                assert.equal(unranged.supportedInDetails(cr.version("10")).supported, false);
                assert.equal(unranged.supportedInDetails(cr.version("11")).supported, false);
                assert.equal(unranged.supportedInDetails(cr.current()).supported, false);
                assert.equal(unranged.supportedInDetails(cr.releases.at(-1)).supported, false);
                const ranged = new SupportStatement({ version_added: "≤5", version_removed: "10" }, cr);
                assert.equal(ranged.supportedInDetails(cr.version("10")).supported, false);
                assert.equal(ranged.supportedInDetails(cr.version("11")).supported, false);
                assert.equal(ranged.supportedInDetails(cr.current()).supported, false);
                assert.equal(ranged.supportedInDetails(cr.releases.at(-1)).supported, false);
            });
        });
    });
});
