...
describe("Release", function () {
    describe("toString()", function () {
        it("returns something useful", function () {
            const latestNonPreview = browser("chrome").releases.at(-2);
            assert(latestNonPreview.toString().startsWith("[Chrome 1"), latestNonPreview.toString());
        });
    });
    describe("date (getter)", function () {
        it("get the date as a Temporal PlainDate", function () {
            const release = browser("chrome").releases.find((r) => r.version === "100");
            assert(release.date !== null);
            assert.equal(Temporal.PlainDate.compare(release.date, Temporal.PlainDate.from("2022-03-29")), 0);
        });
    ...
    describe("compare()", function () {
        it("throws when comparing between two browsers", function () {
            ...
            assert.throws(() => cr.version("100").inRange(ed.version("79"), cr.version("125")), Error);
        });
    ...
    describe("#isPrerelease()", function () {
        ...
        it("returns true for the next release", function () {
            ...
            const chromeNext = chrome.releases.at(chrome.releases.indexOf(chromeCurrent) + 1);
    ...
    describe("inRange()", function () {
        ...
        it("handles closed ranges", function () {
            ...
            assert.equal(cr.version("1").inRange(cr.version("1"), cr.version("125")), true);
            // End of range is exclusive
            assert.equal(cr.version("20").inRange(cr.version("1"), cr.version("20")), false);
            assert.equal(cr.version("1").inRange(cr.version("10"), cr.version("15")), false);
