    ...
    describe("Feature", function () {
        ...
        describe("flatSupportedIn()", function () {
            ...
            it("returns false for qualified support", function () {
                const cr = browser("chrome");
                assert.equal(feature("css.properties.line-clamp").supportedIn(cr.version("100")), false); // { version_added: "…", "prefix": "-webkit-" }
            });
            it("returns false for wholly unsupported", function () {
                const fx = browser("firefox");
                assert.equal(feature("api.Accelerometer").supportedIn(fx.current()), false); // { version_added: false }
            });
        ...
        describe("supportedIn()", function () {
            it("returns support for features supported with and without qualification", function () {
                const cr = browser("chrome");
                // { version_added: "…" }
                const bgColor = feature("css.properties.background-color").supportedInDetails(cr.version("100"));
                assert.equal(bgColor.length, 1);
                assert.equal(bgColor[0]?.supported, true);
                // { version_added: "…", prefix: "-webkit-" }
                const lineClamp = feature("css.properties.line-clamp").supportedInDetails(cr.version("100"));
            ...
            it("returns mixed results for (un)prefixed features", function () {
                const fx = browser("firefox");
                const actual = feature("css.types.gradient.repeating-linear-gradient").supportedInDetails(fx.version("100"));
