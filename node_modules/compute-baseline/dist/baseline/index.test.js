...
describe("computeBaseline", function () {
    ...
    it("returns something sensible for the most complex features", function () {
        ...
        assert.equal(result["api.Document.visibilitychange_event"]?.baseline, "high");
    ...
    it("returns something sensible for the least complex features", function () {
        ...
        assert.equal(result["css.properties.counter-reset.reset_does_not_affect_siblings"]
            ?.baseline, false);
        assert.equal(result["html.global_attributes.exportparts"]?.baseline, "high");
    ...
    it("finds discrepancies with ancestors (checkAncestors)", function () {
        ...
        assert.notEqual(result.baseline_low_date?.toString(), resultWithAncestors.baseline_low_date?.toString());
        chai.expect(result).to.matchSnapshot();
...
describe("keystoneDateToStatus()", function () {
    it('returns "low" for date 1 year before cutoff date', function () {
        const status = keystoneDateToStatus("2020-01-01", Temporal.PlainDate.from("2021-01-01"), false);
    ...
    it('returns "high" for date 3 years before cutoff date', function () {
        const status = keystoneDateToStatus("2020-01-01", Temporal.PlainDate.from("2024-01-01"), false);
    ...
    it("returns false for null dates", function () {
        const status = keystoneDateToStatus(null, Temporal.PlainDate.from("2020-01-01"), false);
    ...
    it("returns false for discouraged (deprecated, obsolete, etc.) features", function () {
        const status = keystoneDateToStatus("2020-01-01", Temporal.PlainDate.from("2020-01-01"), true);
        assert.equal(status.baseline, false);
        assert.equal(status.baseline_low_date, null);
        assert.equal(status.baseline_high_date, null);
    });
